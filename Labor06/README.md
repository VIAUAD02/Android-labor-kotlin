...
> A `DialogFragment`-et az `androidx.fragment.app` csomagból, az `AlertDialog`-ot pedig az `androidx.appcompat.app` csomagból importáljuk! Ha az auto-import beimportálja az `android.R` package-t, azt töröljük ki, és győződjünk meg, hogy a `hu.bme.aut.android.shoppinglist.R` package legyen importálva helyette. Továbbá a rossz import miatt az *onCreateDialog*-ban az ArrayAdapternek átadott `android.R.layout.simple_spinner_dropdown_item` elejéről az `android` kitörlődhet, ezt javítsuk.
